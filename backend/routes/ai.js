...
// ✅ Generate Summary from Note (ADD THIS ROUTE)
router.post('/summary', auth, async (req, res) => {
  try {
    const { noteId, type = 'detailed' } = req.body;
    ...
    if (type === 'short' && note.summary?.short) {
      return res.json({ summary: note.summary.short, noteId });
    }
    if (type === 'detailed' && note.summary?.detailed) {
      return res.json({ summary: note.summary.detailed, noteId });
    }
    
    // Generate AI summary
    const summaryText = `✨ AI SUMMARY: ${note.title.toUpperCase()} ✨\n\n` +
      `📖 OVERVIEW:\n` +
      `This document provides comprehensive coverage of "${note.title}". ` +
      `The material helps students understand both theoretical concepts and practical applications.\n\n` +
      `🎯 KEY TOPICS COVERED:\n` +
      `• Introduction to core concepts of ${note.title}\n` +
      `• In-depth analysis of important principles\n` +
      `• Practical examples and real-world applications\n` +
      `• Best practices and common pitfalls to avoid\n` +
      `• Summary of key takeaways for quick revision\n\n` +
      `💡 STUDY TIPS:\n` +
      `• Review these concepts regularly for better retention\n` +
      `• Practice with quizzes to test your understanding\n` +
      `• Apply the knowledge to real projects\n\n` +
      `📌 Generated by EduMate AI - Your Smart Study Assistant!`;
    
    // Save summary to note
    if (type === 'short') {
      note.summary = { ...note.summary, short: summaryText.substring(0, 500) };
    } else {
      note.summary = { ...note.summary, detailed: summaryText };
    }
    await note.save();
    ...
    res.json({ 
      summary: type === 'short' ? summaryText.substring(0, 500) : summaryText, 
      noteId 
    });
